...
public class TypePresenter extends BasePresenter<TypeView> {
        ...
        WanService.getTypeTagData()
                ...
                .subscribe(new RxObserver<List<TypeTagVO>>() {
                    ...
                    public void _onNext(List<TypeTagVO> typeTagVOS) {
                        ...
                        setTabUI();
                        mTabSelect=0;
                        getServerData(mTagDatas.get(0).getChildrens().get(0).getId());
                    }
                });
    }
    ...
    private void setTagUI(int position) {
        ...
        for (int i = 0; i < mTagDatas.get(position).getChildrens().size(); i++) {
            View view = LinearLayout.inflate(mActivity, R.layout.item_tag_layout, null);
            TextView tvItem = (TextView) view.findViewById(R.id.tv_item);
            tvItem.setText(mTagDatas.get(position).getChildrens().get(i).getName());
            llTag.addView(view);
            tagTvs.add(tvItem);
            ...
            view.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    mId = mTagDatas.get(position).getChildrens().get(finalI).getId();
                    getServerData(mId);
                }
            });
        }
